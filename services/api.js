...
// ---------------------------------------------------------------------------
// React Native does not persist cookies, so the backend's HttpOnly `visitor_id`
// cookie never survives on native. The app therefore mints its own id once,
// keeps it in AsyncStorage, and sends it as `X-Visitor-Id` — which the API
// accepts as an equivalent transport (backend middleware/auth.js →
// attachVisitorId). This is what lets logged-OUT mobile users receive
// personalized recommendations instead of the generic trending list.
const VISITOR_ID_KEY = 'eskan_visitor_id';
const UUID_PATTERN = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;
let cachedVisitorId = null;

// UUID v4 shape from Math.random. Not cryptographically strong — `expo-crypto`
// would be, but it is not a dependency here and this id grants access to
// nothing beyond an anonymous recommendation feed. 122 random bits make a
// practical collision (which would merge two guests' profiles) negligible.
...
export const getVisitorId = async () => {
  ...
  try {
    ...
    // Storage unavailable: use a run-lifetime id so this session still
    // personalizes, even though it will not survive an app restart.
    cachedVisitorId = cachedVisitorId || generateVisitorId();
    return cachedVisitorId;
  }
};
...
api.interceptors.request.use(
  async (config) => {
    ...
    // Anonymous personalization identity. Sent on every request so both view
    // telemetry and the recommendation read resolve to the same visitor.
...
const properties = {
  ...
  // Server-ranked and personalized per visitor; `limit` is honoured by the API.
  getRecommended: (limit = 10) => api.get('/properties/recommended', { params: { limit } }),
  create: (data) => {
...
const propertyViews = {
  recordView: async (propertyId) => {
    try {
      // Public telemetry route. NOT `/property-views/:id` — that one is behind
      // requireAuth + requireRole('admin') and 401s for every real user, which
      // is why no mobile view was ever recorded. The backend dedups to one row
      // per visitor per UTC day, so calling this on every open is safe.
      const response = await api.post(`/properties/${propertyId}/views`, {}, {
        validateStatus: (status) => status === 200 || status === 201 || status === 401 || status === 404
      });
  ...
  getViewCount: async (propertyId) => {
    try {
      ...
      const fetchPromise = (async () => {
        const response = await api.get(`/properties/${propertyId}/views/count`, {
